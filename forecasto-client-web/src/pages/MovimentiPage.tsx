...
export function MovimentiPage() {
  ...
  const confirmBulkDelete = async () => {
    ...
    try {
      // Group IDs by workspace so we emit at most one bulk call + one SSE event per workspace.
      const idsByWorkspace = new Map<string, string[]>()
      for (const record of recordsToDelete) {
        const list = idsByWorkspace.get(record.workspace_id) || []
        list.push(record.id)
        idsByWorkspace.set(record.workspace_id, list)
      }

      const results = await Promise.all(
        Array.from(idsByWorkspace.entries()).map(([workspaceId, ids]) =>
          bulkDeleteRecords({ ids, workspaceId })
        )
      )

      const deleted = results.reduce((sum, r) => sum + r.deleted_count, 0)
      const failed = results.reduce((sum, r) => sum + r.errors.length, 0)

      if (failed === 0) {
        toast({ title: 'Record eliminati', description: `${deleted} record eliminati`, variant: 'success' })
      } else if (deleted === 0) {
        toast({ title: 'Errore', description: 'Nessun record eliminato.', variant: 'destructive' })
      } else {
        toast({ title: 'Eliminazione parziale', description: `${deleted} eliminati, ${failed} falliti.`, variant: 'destructive' })
      }
    } catch (error) {
      const axiosError = error as AxiosError<{ error?: string; message?: string }>
      const message = axiosError.response?.data?.error || 'Errore durante l\'eliminazione.'
